...
namespace TrimFuel.Web.Admin.Logic.Reports.JMB
{
    ...
    {
        private string JMB_INFO_URL = "http://www.biggerbidder.com/livesupport/login.php";
        private string JMB_INFO_URL_TEST = "http://localhost/gateways/jmb_info.aspx";
        private string JMB_INFO_POST_TEMPLATE = "username=triglecrm&password=Tra8a5ap!cha96ye&by=id&for={0}&page=userXML";

        public SaleChargebackReport(string caseNumber, string arnNumber, string companyName, string companyAddress1, string companyAddress2, string assertigyMIDID, ChargeHistoryEx ch, Billing b, string internalID)
        {
        ...
        private void GetJMBInfo()
        {
            if (!string.IsNullOrEmpty(InternalID))
            {
                try
                {
                    ...
                    string response = wc.UploadString(url, string.Format(JMB_INFO_POST_TEMPLATE, InternalID));
        ...
        {
            //string path = System.Web.Hosting.HostingEnvironment.MapPath(string.Format("~/MidImages/" + TrimFuel.Business.Config.Current.APPLICATION_ID + "/usps_{0}.png", AssertigyMIDID));
        ...
        private void GenerateMainDocumentPart1Content(MainDocumentPart mainDocumentPart1)
        {
            ...
            TableRow tableRow17 = new TableRow() { RsidTableRowAddition = "00000000" };

            TableRowProperties tableRowProperties17 = new TableRowProperties();
            TableCellSpacing tableCellSpacing18 = new TableCellSpacing() { Width = "15", Type = TableWidthUnitValues.Dxa };

            tableRowProperties17.Append(tableCellSpacing18);
            ...
            Paragraph paragraph17 = new Paragraph() { RsidParagraphAddition = "00000000", RsidRunAdditionDefault = "006160F6" };

            ParagraphProperties paragraphProperties17 = new ParagraphProperties();
            ParagraphStyleId paragraphStyleId4 = new ParagraphStyleId() { Val = "NormalWeb" };

            paragraphProperties17.Append(paragraphStyleId4);
            ...
            tableRowProperties17.Append(tableCellSpacing18_1);
            ...
            tableCell23.Append(CreateParagraph("We are writing to issue a dispute regarding this chargeback request, as per the terms of sale the customer ordered a product which was shipped and delivery confirmed. The customer has never requested cancellation and the customer still has possession of the merchandise. Please find attached the following items:"));
            ...
            tableCell32.Append(CreateParagraph("Thank you,"));
            tableCell32.Append(CreateParagraph(""));
            tableCell32.Append(CreateParagraph(CompanyName));
            tableCell32.Append(CreateParagraph(CompanyAddress1));
            tableCell32.Append(CreateParagraph(CompanyAddress2));
            ...
            body1.Append(CreateHeaderParagraph("Transaction Information", false));
            ...
            body1.Append(CreateParagraph(string.Format("Settlement Amount: {0}", ChargeHistory.Amount.Value.ToString("0.00"))));
            ...
            body1.Append(CreateHeaderParagraph("Authorization Information", false));
            ...
            body1.Append(CreateParagraph(string.Format("Account Currency: {0}", "US Dollar (USD)")));
            body1.Append(CreateParagraph(string.Format("Transaction Date: {0}", ChargeHistory.ChargeDate)));

            cardBrand = "N/A";
            if (Billing.PaymentTypeID == 2)
                cardBrand = "Visa";
            else if (Billing.PaymentTypeID == 3)
            ...
            cardExpiryDate = "";
            if (Billing.ExpMonth != null && Billing.ExpYear != null)
            {
                year = "";
                if (Billing.ExpYear > 1000)
                    year = Billing.ExpYear.Value.ToString().Substring(2);
                else
                    year = Billing.ExpYear.Value.ToString("00");

                cardExpiryDate = string.Format("{0}/{1}", Billing.ExpMonth.Value.ToString("00"), year);
            }                
            ...
            cardEnding = "";
            cardBin = "";
            decrCard = Billing.CreditCardCnt.DecryptedCreditCard;
            if (!string.IsNullOrEmpty(decrCard) && decrCard.Length >= 6)
            {
                cardEnding = decrCard.Substring(decrCard.Length - 4, 4);
                cardBin = decrCard.Substring(0, 6);
            }

            body1.Append(CreateParagraph(string.Format("Card Ending: {0}", cardEnding)));
            body1.Append(CreateParagraph(string.Format("Card Bin: {0}", cardBin)));
            ...
            body1.Append(CreateHeaderParagraph("Customer Information", false));
            ...
            {
                body1.Append(CreatePageBreak());
                if (JMBInfo.login_history != null)
                {
                    body1.Append(CreateHeaderParagraph("Login/logout times", false));
                    body1.Append(CreateLoginInfoTable(JMBInfo.login_history.loginList));
                }
                if (JMBInfo.purchased_bidpacks != null)
                {
                    body1.Append(CreateHeaderParagraph("Purchased bidpack history", false));
                    body1.Append(CreateBidpackHistoryTable(JMBInfo.purchased_bidpacks.bidpackList));
                }
                if (JMBInfo.auctions_bid_on != null)
                {
                    body1.Append(CreateHeaderParagraph("Auctions taken part in", false));
                    body1.Append(CreateAuctionsInfoTable(JMBInfo.auctions_bid_on.auctionList));
                }
            }            
        ...
        public Paragraph CreateHeaderParagraph(string text, bool pageBreak)
        {
            Paragraph paragraph1 = new Paragraph();
            ...
            if (pageBreak)
            {
                run1.Append(new LastRenderedPageBreak());
            }
            run1.Append(text1);
        ...
        private Table CreateLoginInfoTable(IList<login> info)
        {
            Table table1 = new Table();
            ...
            tableRow1.Append(tableRowProperties1);
            tableRow1.Append(CreateCell("Login Time", true));
            tableRow1.Append(CreateCell("Logout Time", true));

            table1.Append(tableProperties1);
            table1.Append(tableGrid1);
            table1.Append(tableRow1);

            foreach (login item in info)
            {
                TableRow tableRow2 = new TableRow() { RsidTableRowAddition = "00000000" };
                ...
                tableRow2.Append(tableRowProperties2);
                tableRow2.Append(CreateCell(item.login_time, false));
                tableRow2.Append(CreateCell(item.logout_time, false));
        ...
        private Table CreateBidpackHistoryTable(IList<bidpack> info)
        {
            Table table1 = new Table();
            ...
            tableRow1.Append(tableRowProperties1);
            tableRow1.Append(CreateCell("Date", true));
            tableRow1.Append(CreateCell("Amount", true));
            tableRow1.Append(CreateCell("Bids left", true));
            ...
            foreach (bidpack item in info)
            {
                ...
                tableRow2.Append(tableRowProperties2);
                tableRow2.Append(CreateCell(item.date_purchased, false));
                tableRow2.Append(CreateCell(item.purchase_amount, false));
                tableRow2.Append(CreateCell(item.bids_left, false));
        ...
        private Table CreateAuctionsInfoTable(IList<auction> info)
        {
            Table table1 = new Table();
            ...
            tableRow1.Append(tableRowProperties1);
            tableRow1.Append(CreateCell("Product Name", true));
            tableRow1.Append(CreateCell("Start Date", true));
            tableRow1.Append(CreateCell("End Date", true));
            tableRow1.Append(CreateCell("Bidding Outcome", true));
            tableRow1.Append(CreateCell("Total Bids Placed", true));
            ...
            foreach (auction item in info)
            {
                if (item.auction_details != null)
                {
                    ...
                    tableRow2.Append(tableRowProperties2);
                    tableRow2.Append(CreateCell(HttpUtility.HtmlDecode(item.auction_details.productname), false));
                    tableRow2.Append(CreateCell(item.auction_details.startdate, false));
                    tableRow2.Append(CreateCell(item.auction_details.enddate, false));
                    tableRow2.Append(CreateCell(item.auction_details.bidding_outcome, false));
                    tableRow2.Append(CreateCell(item.auction_details.total_bids_placed, false));
        ...
        private TableCell CreateCell(string text, bool header)
        {
            TableCell tableCell1 = new TableCell();
            ...
            if (header)
            {
                tableCell1.Append(CreateHeaderParagraph(text, false));
            }
            else
            {
                tableCell1.Append(CreateParagraph(text));
            }            
